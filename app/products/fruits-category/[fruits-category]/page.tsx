...
const DetailFruitCategoriesPage: NextPage = (): React.ReactElement => {
  ...
  const { data, onGetFruitByCategory }: any = useFruitByCategory(params['fruits-category'] as string)
        ...
        <div className="flex flex-col items-center py-[24px]">
          ...
          <div>
            <div className="mb-1.5 box-border flex items-center gap-2 px-1 text-[12px]">
              ...
              <span className="truncate-ellipsis">{GARDENER_TEXT} : {garden}</span>
            </div>
            <div className="mb-1.5 box-border flex items-center gap-2 px-1 text-[12px]">
              <Icon color="#699C3A" name="ic_phone_outline" size={20} aria-label="Location Icon" />
              <span className="truncate-ellipsis">{CALL_TEXT} : {phone}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className=''>
      <div className="flex h-[229px] items-start justify-between bg-garden pl-[33px] pr-[100px]">
        <div className="inline-flex flex-col items-start justify-start gap-[5px] py-[30px]">
          ...
          <div className="text-base font-normal leading-normal text-white">Đường kính: Từ....cm đến ....cm<br />Trọng lượng: Từ....kg đến ....kg<br />Màu sắc: xanh, vàng chanh bóng<br />Tay ngón: Có tay ngón, hình dáng đẹp<br />Thời gian trưng : 1-5 tháng<br />Sử dụng: Thăm biếu bạn bè, trưng tết, để thờ,…</div>
        </div>
        <div className="h-[229px] w-[236px]">
          <Image className='!static' src={DemoFruit.src} alt="Gardener Image" layout="fill" objectFit="cover" />
        </div>
      </div>
      <div className="ml-[19px] grid grid-cols-1 gap-4 sm:grid-cols-1 lg:grid-cols-3  2xl:grid-cols-5">

        {data?.items?.map((fruitCate: any, index: number) => (
          <ProductOfGarden
